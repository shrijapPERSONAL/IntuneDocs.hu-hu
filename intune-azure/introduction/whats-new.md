...
További információt [Az iOS oktatási funkcióinak konfigurálása](../configure-devices/how-to-configure-ios-edu-settings.md) című cikkben talál.
...
További információt az [Eszközkorlátozások az Android for Work esetén](../configure-devices/device-restrictions-for-afw.md) című témakörben talál.
...
Az [iOS-re](../manage-apps/ios-lob-app.md) (.ipa-fájlok) és [Androidra](../manage-apps/android-lob-app.md) (.apk-fájlok) készült üzletági (LOB) alkalmazásokat felhasználókhoz vagy eszközökhöz rendelheti hozzá.
...
- **A kezdőképernyőn látható alkalmazások** – Azt határozza meg, mely alkalmazásokat láthatják a felhasználók [iOS-es eszközeik kezdőképernyőjén](../configure-devices/home-screen-settings-for-ios.md). Ez a szabályzat módosítja a kezdőképernyő elrendezését, de nem telepíti a megadott, de nem telepített alkalmazásokat.

- **Kapcsolódás AirPrint-eszközökhöz** – Azt határozza meg, hogy mely [AirPrint-eszközökhöz](../configure-devices/air-print-settings-for-ios-and-macos.md) (hálózati nyomtatókhoz) kapcsolódhatnak az adott iOS-es eszköz végfelhasználói.

- **Kapcsolódás AirPlay-eszközökhöz** – Azt határozza meg, hogy mely [AirPlay-eszközökhöz](../configure-devices/airplay-settings-for-ios-devices.md) (például Apple TV-hez) kapcsolódhatnak az adott iOS-es eszköz végfelhasználói.

- **Egyéni üzenet a zárolási képernyőn** – Az alapértelmezett helyett egyéni üzenetet állíthat be az iOS-es eszköz zárolási képernyőjére. További információt az [Elérhető eszközműveletek](../manage-devices/what-is.md#available-device-actions) című témakörben talál


### <a name="restrict-push-notifications-for-ios-apps----723767---"></a>iOS-alkalmazások leküldéses értesítéseinek korlátozása <!-- 723767 -->

Az Intune-os eszközkorlátozási profilokban már a következő [értesítési beállításokat](../configure-devices/app-notification-settings-for-ios.md) is konfigurálhatja iOS-es eszközökhöz:
...
Az iOS-es eszközkorlátozási profilokban a következő [tartománybeállításokat](../configure-devices/device-restrictions-for-ios.md#domains) konfigurálhatja:
...
Az Apple mennyiségi vásárlásra szolgáló áruházából vásárolt [könyvek szinkronizálhatók](../manage-apps/ios-vpp-apps.md) az Intune-nal, és kioszthatók felhasználóknak.
...
A Samsung KNOX Standard rendszerű eszközökön mostantól használható az Intune [többfelhasználós felügyelete](../enroll-devices/enroll-android-and-knox-standard-devices.md). Ilyenkor a felhasználók Azure Active Directory-beli hitelesítő adataikkal jelentkezhetnek be és ki az eszközön, de az központi felügyelet alatt marad, akár használatban van, akár nem.  A bejelentkezett végfelhasználó hozzáfér az alkalmazásokhoz, és a rá érvényes szabályzatok is működnek. A felhasználói kijelentkezéskor az összes alkalmazásadat törlődik.

### <a name="additional-windows-device-restriction-settings----818566---"></a>További windowsos eszközkorlátozási beállítások <!-- 818566 -->

További [windowsos eszközkorlátozási beállításokat](../configure-devices/device-restrictions-for-windows-10.md) tettünk elérhetővé, például bővült az Edge böngésző támogatása, testre szabható az eszközök zárolási képernyője és Start menüje, szabályozható a Windows Reflektorfény háttérképkészlete, illetve a proxybeállítások.
...
Az új [hibaelhárítási portálon](../manage-users/help-desk.md) az ügyfélszolgálati munkatársak és az Intune-rendszergazdák áttekinthetik a felhasználók és eszközeik listáját, és az Intune technikai problémáinak elhárítását is elvégezhetik.
...
További információt [A Microsoft Intune-eszközfelügyelet ismertetése](../manage-devices/what-is.md) című témakörben talál.
...
Lásd: [Alkalmazás hozzáadása az Intune-hoz](../manage-apps/add-apps.md).
...
Az Intune mindaddig személyes tulajdonúként jelöli meg az összes új eszközt, amíg az [alábbi cikkben](https://docs.microsoft.com/intune/deploy-use/manage-corporate-owned-devices) ismertetett módon a rendszergazda meg nem jelöli azokat céges eszközökként.

### <a name="view-all-actions-on-managed-devices---677150--"></a>Az összes művelet megtekintése a felügyelt eszközökön<!--677150-->
Az új __Eszközműveletek__ jelentés megmutatja, hogy mely felhasználó hajtott végre távoli műveleteket az eszközön (például gyári alaphelyzetbe állítás), valamint megjeleníti az adott művelet állapotát is. Lásd: [Mi az eszközfelügyelet?](https://docs.microsoft.com../manage-devices/what-is.md)
...
Mostantól az eszközkategória külön oszlopként jelenik meg az eszközlistában. A kategória módosítására szintén van lehetőség az Eszköztulajdonságok panel Tulajdonságok szakaszában. Lásd: [Alkalmazás hozzáadása az Intune-hoz](../manage-apps/add-apps.md).
...
Ha a felhasználói eszközök elveszítik a kapcsolatot az Intune-nal, új hibaelhárítási lépések átadásával segítheti a felhasználókat a vállalati erőforrásokhoz való hozzáférés visszaszerzésében. Lásd: [Az eszközök inaktívak, vagy a felügyeleti konzol nem tud kommunikálni velük](../enroll-devices/troubleshoot-device-enrollment.md#devices-are-inactive-or-the-admin-console-cannot-communicate-with-them).
