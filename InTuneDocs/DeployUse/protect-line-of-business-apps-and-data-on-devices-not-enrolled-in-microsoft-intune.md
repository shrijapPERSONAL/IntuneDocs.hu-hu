...
A mobilalkalmazás-kezelési (MAM) szabályzatok segítségével korlátozhatja az adatáthelyezést (például a másolást és a beillesztést), valamint megakadályozhatja, hogy a felhasználók vállalati dokumentumokat mentsenek személyes helyekre, elősegítve ezzel a vállalati adatok védelmét.   A MAM-szabályzatok iOS- vagy Android-rendszeren futó üzleti alkalmazásokon való használatához először a Microsoft Intune alkalmazásburkolóval burkolnia kell az alkalmazást.  Az alkalmazásburkolás az a folyamat, amelynek során kezelési réteggel lát el egy mobilalkalmazást anélkül, hogy a mögöttes alkalmazást módosítaná.  Az alkalmazást a burkolása után MAM-szabályzattal láthatja el, és kioszthatja végfelhasználói között.  

Ez a témakör ismerteti a MAM-szabályzatoknak a **nem felügyelt, alkalmazotti eszközökön** és a **harmadik fél által nyújtott mobileszköz-kezelési (MDM) megoldások** által felügyelt eszközökön futó alkalmazásokhoz való hozzárendelését.  Az **Intune-ban regisztrált eszközökön futó üzletei alkalmazások** előkészítéséhez tekintse meg az [Alkalmazások előkészítése mobilalkalmazás-kezelésre a Microsoft Intune-nal](decide-how-to-prepare-apps-for-mobile-application-management-with-microsoft-intune.md) című cikket.
...
>[!IMPORTANT]  
>Az alkalmazásburkoló ezen verziója, amely az Intune-ban nem regisztrált alkalmazásokat támogatja, a közeljövőben elérhető lesz Private preview verzióban. Ha részt szeretne venni a programban, küldjön levelet a msintuneappsdk@microsoft.com címre további információkért.
